window.addEventListener('DOMContentLoaded', function(){
    ...
    //Калькулятор
    //получаем наши инпуты и превращем их в массив
    const inputs = [...document.querySelectorAll('.counter-block-input')];
    //пишем функцию 
    const recalc = () => {
        //для каждого инпута заменяем буквы на пустые строки
        inputs.forEach(el => el.value = el.value.replace(/[^\d]/g, ''));
        //перебираем массив с инпутами и в коллбэке пишем формулу где а и b - это индексы инпутов
        total.textContent = inputs.reduce((a, b) => (+a.value || 0) + (+b.value || 0)) * 4000 * select.value;
    };
    //вешаем обработчик событий на инпуты
